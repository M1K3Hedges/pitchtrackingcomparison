...
def ACF(sig, W, t, lag): #signal, window, timestep, # of sample shift
    ...
    #print(ac)
    return ac
    #take the signal, window it and multiply it with a copy of itself that's shifted by a # of samples
...
def CMNDF(sig, W, t, lag): 
    ...
    #denom = np.sum([DF(sig, W, t, j + 1) for j in range(lag)])
    #print(denom)
...
def detect_pitch(sig, W, t, fs, bounds, threshold = 0.1): #bounds = limit the range of the search for detect_pitch
    CMNDF_values = [CMNDF(sig, W, t, i) for i in range(*bounds)] #list of all lag values within the bounds
...
def runloop(stream_in):
    window_size = 200 #int(5 / 200 * 44100)
    bounds = [20, 2000]
